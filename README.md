...
three or four different PDFs at the same time. With `top_k=3` the retriever
returns three chunks from whichever single quarter matched best, and the model
answers a comparison question from one document without ever signalling that it
did. This is the most dangerous failure mode in the system, because the output
looks completely normal. 6 gives enough spread. `run_test_questions.py`
explicitly reports how many distinct quarters were retrieved for every
comparison question, so the failure becomes visible instead of silent.
...
Running `python run_test_questions.py --show-chunks` writes the system's actual
answers, the sources retrieved for each, and the raw chunks behind them to
`docs/test_answers.md`. That file is generated rather than committed, because it
records one particular run against a live API key.

Below is the **hand-verified answer key**, read directly out of the four filings.
This is the reference the generated answers are checked against.
